...
# (connect, read) timeouts. Report generation and status queries can take the
# Wiz API well over 30s to answer, so the read budget is generous by default.
...
def _post_with_retries(url: str, description: str, **kwargs) -> requests.Response:
    ...
    retry loop never sees them and the whole fetcher run dies on one slow call.
    """
    attempt = 0
    while True:
        try:
...
def main():
    ...
    # Step 9: Write summary JSON for TUI / create_summary_file
    # Filename is shortened vs check name; run_fetchers maps wiz_issues_report -> wiz_issues.json
