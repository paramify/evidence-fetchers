...
def fetch_incidents(base_url: str, headers: Dict[str, str], from_ts: str, to_ts: str) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        try:
            ...
            print(f"Warning: Incident pagination interrupted: {e}", file=sys.stderr)
            break
...
def fetch_timeline_cells(
    ...
    try:
        ...
        print(f"Warning: Timeline fetch failed for incident {incident_id}: {e}", file=sys.stderr)
        return []
...
def main() -> int:
    ...
    component = "datadog_incident_timelines"
